...
async function showMsg() {
  if ($.errorMsg) return
  allMessage += `账号${$.index}：${$.nickName || $.UserName}\n今：${$.todayIncomeBean}豆 \n昨：${$.incomeBean}豆 \n总：${$.beanCount}(${$.expirejingdou})豆 ${$.message}${$.index !== cookiesArr.length ? '\n\n' : ''}`;
  // if ($.isNode()) {
  //   await notify.sendNotify(`${$.name} - 账号${$.index} - ${$.nickName}`, `账号${$.index}：${$.nickName || $.UserName}\n昨：${$.incomeBean}豆 🐶\n总：${$.beanCount}豆 ${$.message}`, { url: `https://bean.m.jd.com/beanDetail/index.action?resourceValue=bean` })
  // }
  $.msg($.name, '', `${$.index}：${$.nickName || $.UserName}\n今：${$.todayIncomeBean}豆 \n昨：${$.incomeBean}豆\n总：${$.beanCount}(${$.expirejingdou})豆${$.message}`, {"open-url": "https://bean.m.jd.com/beanDetail/index.action?resourceValue=bean"});
}
async function bean() {
      ...
      try {
        if (err) {
          ...
          if (data) {
            ...
            $.message += `\n红包：${$.balance}(${$.expiredBalance})元 🧧\n京喜：${$.jxRed}(${$.jxRedExpire.toFixed(2)})元 🧧\n极速：${$.jsRed}(${$.jsRedExpire.toFixed(2)})元 🧧\n京东：${$.jdRed}(${$.jdRedExpire.toFixed(2)})元 🧧\n健康：${$.jdhRed}(${$.jdhRedExpire.toFixed(2)})元 🧧`;
          } else {
            console.log(`京东服务器返回空数据`)
          }
        }
